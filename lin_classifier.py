...
def cv_kfold(X, y, C, penalty, K, mode):
    ...
    for c in C:
        ...
        # ------------------ IMPLEMENT YOUR CODE HERE:-----------------------------
                ...
                y_val_prob, w = pred_log(logreg, x_train, y_train, x_val, flag = True)
                loss_val_vec[k] = log_loss(y_val, y_val_prob)
...
def odds_ratio(w, X, selected_feat='LB'):
    ...
    """
    # ------------------ IMPLEMENT YOUR CODE HERE:-----------------------------
    feat_ind = X.columns.get_loc(selected_feat)
    odd_ratio = np.exp(w[0][feat_ind])
    odds_vec = 1/np.exp(-w[0][feat_ind]*X[[selected_feat]])
